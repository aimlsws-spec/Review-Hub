...
async function fillForm(user: ReturnType<typeof userEvent.setup>, overrides: Partial<Record<'email' | 'code' | 'password' | 'confirmPassword', string>> = {}) {
  await user.type(screen.getByLabelText(/email address/i), overrides.email ?? 'admin@viralkar.com')
...
describe('ResetPasswordPage', () => {
  ...
  it('submits the reset and redirects to login on success', async () => {
    ...
    expect(mutateMock).toHaveBeenCalledWith(
      { email: 'admin@viralkar.com', code: '123456', password: 'newpassword1', confirmPassword: 'newpassword1' },
      expect.any(Object),
    )
